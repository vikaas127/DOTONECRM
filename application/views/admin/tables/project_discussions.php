...
foreach ($rResult as $aRow) {
    ...
    if (staff_can('edit',  'projects') || staff_can('delete',  'projects')) {
        ...
        if (staff_can('edit',  'projects')) {
            $subject .= '<a href="#" onclick="edit_discussion(this,' . $aRow['id'] . '); return false;" data-subject="' .($aRow['subject']) . '" data-description="' .(clear_textarea_breaks($aRow['description'])) . '" data-show-to-customer="' .($aRow['show_to_customer']) . '">' . _l('edit') . '</a>';
        }
