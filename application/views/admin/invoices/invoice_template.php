    ...
    <div class="panel-body">
        ...
        <div class="row">
            <div class="col-md-6">
                ...
                <!-- ✅ New Field for E-Invoice Number -->
...
<div class="btn-bottom-toolbar text-right">
    ...
    <div class="btn-group dropup">
        ...
        <ul class="dropdown-menu dropdown-menu-right width200">
            <li>
                <button id="save-and-generate-einvoice" class="btn btn-primary invoice-form-submit save-and-generate-einvoice transaction-submit"
                    data-url="<?php echo base_url('GSTAPI/generateEInvoice'); ?>">
                    <?php echo _l('save_and_generate_einvoice'); ?>
                </button>
            </li>
            <script>
    function fetchInvoices() {
        fetch("<?php echo base_url('Invoices_model/getInvoices'); ?>", {method : "GET"})
            .then(response => response.json()) // Convert response to JSON
            .then(data => {
                if (data.status === 'success') {
                    console.log(data.data); // Log data in the console
                    alert('Invoices retrieved successfully!');
                } else {
    ...
    fetchInvoices();
            ...
            <script>
                document.addEventListener("DOMContentLoaded", function() {
                    document.getElementById("save-and-generate-einvoice").addEventListener("click", function(event) {
                        event.preventDefault()

                        var dummyData = {
                            e_invoice_number: "EINV-" + Math.floor(100000 + Math.random() * 900000),
                            irn: "IRN-" + Math.floor(100000 + Math.random() * 900000),
                            qr_code: "Sample QR Code Data " + new Date().getTime(),
                            einvoice_status: "Pending",
                            eway_bill_number: "EWB-" + Math.floor(100000 + Math.random() * 900000),
                            is_e_invoice: 1
                        };

                        // Send AJAX request
                        fetch("<?php echo base_url('Invoices_model/add'); ?>", {
                                method: "POST",
                                headers: {
            ...
            <!-- <script>
                $(document).on('click', '.save-and-generate-einvoice', function(e) {
                    e.preventDefault();
                    var url = $(this).data('GSTAPI/generateEInvoice');
                    ...
                    console.log("🔵 Sending Data:", dummyData); // Console log the data before sending
