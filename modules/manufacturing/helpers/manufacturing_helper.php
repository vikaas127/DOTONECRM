...
function list_manufacturing_permisstion()
{
    log_message('info', 'Fetching list of manufacturing permissions.');
    
    
        $manufacturing_permissions=[];
    $manufacturing_permissions[] = 'manufacturing';
    $manufacturing_permissions[] = 'bill_of_material';
    $manufacturing_permissions[] = 'manufacturing_orders';
    $manufacturing_permissions[] = 'routing';
    //work_order
     $manufacturing_permissions[] = 'work_order';
    $manufacturing_permissions[] = 'work_centers';
    $manufacturing_permissions[] = 'mrp_settings';
   
    
        
    

    log_message('debug', 'Manufacturing permissions list:', $manufacturing_permissions);

    return $manufacturing_permissions;
}

function manufacturing_get_staff_id_manufacturing_permissions()
{
    $CI = &get_instance();
    $array_staff_id = [];
    $index = 0;

    log_message('info', 'Fetching staff IDs with manufacturing permissions.');

    $str_permissions = '';
    foreach (list_manufacturing_permisstion() as $per_value) {
    ...
    log_message('debug', 'Compiled permission string for SQL query:', ['permissions' => $str_permissions]);
    ...
    log_message('debug', 'Query executed:', ['sql' => $sql_where]);
    ...
    if (count($staffs) > 0) {
        ...
        log_message('debug', 'Staff IDs with manufacturing permissions:', $array_staff_id);
    } else {
        log_message('info', 'No staff found with manufacturing permissions.');
    }
...
function manufacturing_get_staff_id_dont_permissions()
{
    $CI = & get_instance();

    $CI->db->where('admin != ', 1);

    if(count(manufacturing_get_staff_id_manufacturing_permissions()) > 0){
        $CI->db->where_not_in('staffid', manufacturing_get_staff_id_manufacturing_permissions());
    }
