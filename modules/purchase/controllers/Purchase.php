...
/**
    ...
    public function commodity_list_add_edit($id='') {
        ...
        log_message('debug', 'Received input data: ' . json_encode($data));
    
        if ($data) {
            if (!isset($data['id'])) {
                log_message('debug', 'Adding a new commodity item.');
                
                if (!empty($data['hs_code'])) {
                    $data['hs_code'] = $data['hs_code'];
                } else {
                    $data['hs_code'] = null;
                }
                ...
                if ($ids) {
                    log_message('debug', 'Commodity item added successfully. ID: ' . $ids);
    
                    // Fetch saved commodity item to log all columns
                    $this->db->where('id', $ids);
                    $query = $this->db->get(db_prefix().'items');
                    $item_details = $query->row_array();
    
                    log_message('debug', 'Saved Commodity Item Details: ' . json_encode($item_details));
                ...
                log_message('error', 'Commodity item could not be added.');
                echo json_encode([
                ...
                $id = $data['id'];
                unset($data['id']);
                log_message('debug', 'Updating commodity item. ID: ' . $id);
                ...
                /* Log updated commodity details */
                $this->db->where('id', $id);
                $query = $this->db->get(db_prefix().'items');
                $updated_item = $query->row_array();
    
                log_message('debug', 'Updated Commodity Item Details: ' . json_encode($updated_item));
    
                if ($success) {
                    log_message('debug', 'Commodity item updated successfully. ID: ' . $id);
                    $message = _l('updated_successfully');
                    set_alert('success', $message);
                } else {
                    log_message('error', 'Commodity item update failed. ID: ' . $id);
                }
