...
$aColumns = [
	...
	db_prefix() . 'ware_commodity_type.commondity_name as commondity_name',
...
if (get_status_modules_wh('purchase')) {
	$result = data_tables_init($aColumns, $sIndexColumn, $sTable, $join, $where, [db_prefix() . 'items.id', db_prefix() . 'items.description', db_prefix() . 'items.unit_id', db_prefix() . 'items.commodity_code', db_prefix() . 'items.thickness', db_prefix() . 'items.commodity_barcode', db_prefix() . 'items.commodity_type', db_prefix() . 'items.warehouse_id', db_prefix() . 'items.origin', db_prefix() . 'items.color_id', db_prefix() . 'items.style_id', db_prefix() . 'items.model_id ',  db_prefix() . 'items.paperwork_id ',db_prefix() . 'items.size_id', db_prefix() . 'items.rate', db_prefix() . 'items.tax', db_prefix() . 'items.group_id', db_prefix() . 'items.long_description', db_prefix() . 'items.sku_code', db_prefix() . 'items.sku_name', db_prefix() . 'items.hs_code',db_prefix() . 'items.sort_name', db_prefix() . 'items.sub_group', db_prefix() . 'items.color', db_prefix() . 'items.guarantee', db_prefix().'items.profif_ratio', db_prefix().'items.without_checking_warehouse',  db_prefix().'items.parent_id', db_prefix().'items.tax2', db_prefix().'items.can_be_sold', db_prefix().'items.can_be_purchased', db_prefix().'items.can_be_manufacturing', db_prefix().'items.can_be_inventory', db_prefix().'ware_style_type.style_code', db_prefix().'ware_style_type.style_barcode', db_prefix().'ware_style_type.style_name', db_prefix().'ware_style_type.note' , db_prefix().'ware_body_type.body_code', db_prefix().'ware_body_type.body_name', db_prefix().'ware_body_type.note'  , db_prefix().'ware_size_type.size_code', db_prefix().'ware_size_type.size_name', db_prefix().'ware_size_type.size_symbol', db_prefix().'ware_size_type.note' , db_prefix().'ware_color.color_code', db_prefix().'ware_color.color_name', db_prefix().'ware_color.color_hex', db_prefix().'ware_color.note', db_prefix().'items.from_vendor_item' ]);
}else{

	$result = data_tables_init($aColumns, $sIndexColumn, $sTable, $join, $where, [db_prefix() . 'items.id', db_prefix() . 'items.description', db_prefix() . 'items.unit_id', db_prefix() . 'items.commodity_code',db_prefix() . 'items.thickness',  db_prefix() . 'items.commodity_barcode', db_prefix() . 'items.commodity_type', db_prefix() . 'items.warehouse_id', db_prefix() . 'items.origin', db_prefix() . 'items.color_id', db_prefix() . 'items.style_id', db_prefix() . 'items.model_id',db_prefix() . 'items.paperwork_id ', db_prefix() . 'items.size_id', db_prefix() . 'items.rate', db_prefix() . 'items.tax', db_prefix() . 'items.group_id', db_prefix() . 'items.long_description', db_prefix() . 'items.sku_code',  db_prefix() . 'items.sku_name',db_prefix() . 'items.sort_name',db_prefix() . 'items.hs_code',   db_prefix() . 'items.sub_group', db_prefix() . 'items.color', db_prefix() . 'items.guarantee', db_prefix().'items.profif_ratio', db_prefix().'items.without_checking_warehouse', db_prefix().'items.parent_id', db_prefix().'items.tax2', db_prefix().'items.can_be_sold', db_prefix().'items.can_be_purchased', db_prefix().'items.can_be_manufacturing', db_prefix().'items.can_be_inventory', db_prefix().'ware_style_type.style_code', db_prefix().'ware_style_type.style_barcode', db_prefix().'ware_style_type.style_name', db_prefix().'ware_style_type.note' , db_prefix().'ware_body_type.body_code', db_prefix().'ware_body_type.body_name', db_prefix().'ware_body_type.note'  , db_prefix().'ware_size_type.size_code', db_prefix().'ware_size_type.size_name', db_prefix().'ware_size_type.size_symbol', db_prefix().'ware_size_type.note' , db_prefix().'ware_color.color_code', db_prefix().'ware_color.color_name', db_prefix().'ware_color.color_hex', db_prefix().'ware_color.note' ]);
}
	...
	foreach ($rResult as $aRow) {
		...
		for ($i = 0; $i < count($aColumns); $i++) {
			...
			if ($aColumns[$i] == 'commodity_code') {
				...
				$code .= '<div class="row-options" style="width:150px">';
				...
				$full_desc = strip_tags($aRow['description']);
				$short_desc = mb_substr($full_desc, 0, 35);
				$is_truncated = mb_strlen($full_desc) > 35;
				$display_text = $is_truncated ? $short_desc . '...' : $short_desc;
			
				$style = 'width:150px; display:inline-block; word-wrap:break-word; white-space:normal;';
				$tooltip = htmlspecialchars($full_desc, ENT_QUOTES);
			
				if (isset($arr_inventory_min[$aRow['id']]) && $arr_inventory_min[$aRow['id']] == true) {
					// Inventory is low: add red color and tooltip
					$_data = '<a href="#" class="custom-tooltip text-danger" data-tooltip="' . $tooltip . '" style="' . $style . ' color:red;" onclick="show_detail_item(this);return false;" data-name="' . $full_desc . '" data-warehouse_id="' . $aRow['warehouse_id'] . '" data-commodity_id="' . $aRow['id'] . '">' . $display_text . '</a>';
				} else {
					// Normal case
					$_data = '<a href="#" class="custom-tooltip" data-tooltip="' . $tooltip . '" style="' . $style . '" onclick="show_detail_item(this);return false;" data-name="' . $full_desc . '" data-commodity_id="' . $aRow['id'] . '">' . $display_text . '</a>';
				}
			}
			elseif($aColumns[$i] == 'sku_code'){
				...
				}elseif ($aColumns[$i] == 'commondity_name') {
				$_data = $aRow['commondity_name'];
